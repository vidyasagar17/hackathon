...
/** Robo's seat and the fact it calls, laid out as cards with a large sign between them. */
...
/**
 * Multiplication Shootout on the game table: Robo calls a fact and the student answers it on the
 * keypad, digits filling left to right like a calculator. After the answer is checked, the verdict
 * and, on request, the hint with its diagnosed pattern sit in a panel beside the answer.
 */
function MultiplicationShootoutPage() {
  ...
  const spoken = answering
    ? `Robo calls ${spokenFact(state.fact)}. Type your answer.`
    : result.correct
